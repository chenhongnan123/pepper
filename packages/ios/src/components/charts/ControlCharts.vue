      ...
      >
        ...
        >
          <div v-if="cusumtestlist">
            {{ $t('dataAlarmDetail.chart.testcusum')}}
            {{ $t('dataAlarmDetail.chart.testfailed') }}
            {{ cusumtestlist }}
          </div>
        </div>
        ...
        >
          <div v-if="zonetestlist">
            {{ $t('dataAlarmDetail.chart.testzone')}}
            {{ $t('dataAlarmDetail.chart.testfailed') }}
            {{ zonetestlist }}
          </div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    Highcharts.chart('valueonechart', this.optionsOne);
    Highcharts.chart('valuetwochart', this.optionsTwo);
    Highcharts.chart('valuethreechart', this.optionsThree);
  },
  methods: {
    ...
    getYAxisTitle(sourcedata, isfirst, issecond, algorithm) {
      ...
      switch (algorithm) {
        // 单值
        case 'i_chart':
          return this.$t('dataAlarmDetail.chart.indiviualvalue');
        case 'mr_chart':
          return this.$t('dataAlarmDetail.chart.movingrange');
        case 'z-mr_chart':
          return isfirst ? this.$t('dataAlarmDetail.chart.standardeddata') : this.$t('dataAlarmDetail.chart.movingrange');
        case 'i-mr_chart':
          return isfirst ? this.$t('dataAlarmDetail.chart.indiviualvalue') : this.$t('dataAlarmDetail.chart.movingrange');
        // 子组
        case 'xbar-r_chart':
          return isfirst ? this.$t('dataAlarmDetail.chart.samplemean') : this.$t('dataAlarmDetail.chart.samplerange');
        case 'xbar-s_chart':
          return isfirst ? this.$t('dataAlarmDetail.chart.samplemean') : this.$t('dataAlarmDetail.chart.samplestandarddeviation');
        case 'xbar_chart':
          return this.$t('dataAlarmDetail.chart.samplemean');
        case 's_chart':
          return this.$t('dataAlarmDetail.chart.samplestandarddeviation');
        case 'r_chart':
          return this.$t('dataAlarmDetail.chart.samplerange');
        // zone控制图无标题
        case 'zone_chart':
          return '';
        // 仅i-mr-r/s算法有三张图
        case 'i-mr-rs_chart':
          // eslint-disable-next-line
          return isfirst ? this.$t('dataAlarmDetail.chart.subgroupmean')
            // eslint-disable-next-line
            : (issecond ? this.$t('dataAlarmDetail.chart.mrsubgroupmean')
              // eslint-disable-next-line
              : (chartUsed === 'r_chart' ? this.$t('dataAlarmDetail.chart.samplerange')
                // eslint-disable-next-line
                : this.$t('dataAlarmDetail.chart.samplestandarddeviation')));
        // 属性
        case 'p_chart':
          return this.$t('dataAlarmDetail.chart.proportion');
        case 'laney-p_chart':
          return this.$t('dataAlarmDetail.chart.proportion');
        case 'u_chart':
          return this.$t('dataAlarmDetail.chart.samplecountperunit');
        case 'laney-u_chart':
          return this.$t('dataAlarmDetail.chart.samplecountperunit');
        case 'c_chart':
          return this.$t('dataAlarmDetail.chart.samplecount');
        case 'np_chart':
          return this.$t('dataAlarmDetail.chart.samplecount');
        // 时间加权
        case 'ewma_chart':
          return this.$t('dataAlarmDetail.chart.ewma');
        case 'ma_chart':
          return this.$t('dataAlarmDetail.chart.movingaverage');
        case 'cumsum_chart':
          return this.$t('dataAlarmDetail.chart.cumulativesum');
        // 多变量
        case 'tsqrd_chart':
          return this.$t('dataAlarmDetail.chart.tsqurd');
        case 'genvar_chart':
          return this.$t('dataAlarmDetail.chart.generalizedvariance');
        case 'tsqrd-gv_chart':
          return isfirst ? this.$t('dataAlarmDetail.chart.tsqurd') : this.$t('dataAlarmDetail.chart.generalizedvariance');
        default:
          return '';
      }
    },
    ...
    getTestContent(points, params, chartData, isfirst, issecond) {
      ...
      switch (name) {
        case 'oocl':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.onepoint', { num: params?.tests?.onepoint?.value })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'kos':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.npointssameside', { num: params?.tests?.npointssameside?.value })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'kid':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.npointsall', { num: params?.tests?.npointsall?.value })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'kud':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.npointsupdown', { num: params?.tests?.npointsupdown?.value })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'os2std':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.pointsbiggerthan2', { num: params?.tests?.pointsbiggerthan2?.value, num2: (params?.tests?.pointsbiggerthan2?.value || 0) + 1 })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'os1std':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.pointsbiggerthan1', { num: params?.tests?.pointsbiggerthan1?.value, num2: (params?.tests?.pointsbiggerthan1?.value || 0) + 1 })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'kin1std':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.npoints1', { num: params?.tests?.npoints1?.value })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        case 'koo1std':
          return `${this.$t('dataAlarmDetail.chart.test', { num })}${this.$t('dataAlarmDetail.options.npointsbiggerthan1', { num: params?.tests?.npointsbiggerthan1?.value })}${this.$t('dataAlarmDetail.chart.testfailed')}${valueStr}`;
        default:
          return '';
      }
    },
