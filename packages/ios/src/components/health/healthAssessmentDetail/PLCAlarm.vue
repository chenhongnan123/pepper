<template>
    <div class="warp">
        <!-- <div class="alart-range pa-2 px-4">30天内异常波动：</div> -->
        <v-carousel v-if="allAlarmList.length > 0"
            cycle height="370" delimiter-icon="mdi-circle" hide-delimiter-background
            hide-delimiters>
            <v-carousel-item v-for="(item, i) in allAlarmList" :key="i">
                <div class="carousel-title d-flex align-center px-12 rounded pt-4">
                    ...
                    <div class="card-content d-flex justify-center align-center">
                        {{ `${$t('repairSuggestionDetail.title.PLCAlarm')}` }}
                    </div>
                    <!-- 分页 -->
                ...
                <template>
                    <PlcAlarmList class=" px-8" :alarmData="item" :height="320"></PlcAlarmList>
                </template>
            </v-carousel-item>
        ...
        <template v-else>
          <div class="carousel-title d-flex align-center rounded px-12 pt-6">
                    <div class="carousel-card-icon d-flex justify-start align-center mr-2">
                        <v-icon class="card-icon d-flex justify-center align-center">
                            mdi-alert</v-icon>
                    </div>
...
import PlcAlarmList from './PLCAlarmList.vue';

export default {
  name: 'PLCAlarm',
  components: { PlcAlarmList },
  data() {
    return {
  ...
  computed: {
    ...
    ...mapState('statePrediction', ['recordList', 'algorithmTypeList', 'algorithmIconList', 'algorithmPlcList']),
    barHeight() {
      return this.height - this.statusHeight;
    },
    allAlarmList() {
      // return this.algorithmPlcList.concat(this.recordList);
      return this.algorithmPlcList;
    },
  },
  created() { },
  methods: {
