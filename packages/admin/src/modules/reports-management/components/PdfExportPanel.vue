...
export default {
  ...
  computed: {
    ...
    ...mapState('reportsManagement', ['reportMapping', 'dateRange', 'agChartId']),
    aggType() {
  ...
  methods: {
    submitFormHandler({ agGridApi, agColumnApi }) {
      ...
      printDoc(printParams, fileName, agGridApi, agColumnApi, this.agChartId);
    },
  },
  watch: {
    reportMapping() {
  ...
  mounted() {
    ...
    img.src = require('@/assets/logo/logo-dark.png');
    img.onload = () => {
