  ...
  >
    <v-card-title>
      <div class="font-weight-bold">
        {{ $t('statePrediction.title.equipmentHealthDetail') }}
      </div>
    </v-card-title>
    <v-card-text
      class="pb-0"
        ...
        >
          <div class="d-flex flex-column align-center"
            ...
            <div :style="`height: ${radarHeight}px;`">
              <!-- <health-annulus-chart v-bind="$attrs"
                :height="radarHeight"
              /> -->
              <v-progress-circular :style="`font-size:${radarHeight*0.3}px;`"
                :value="healthAverage" :size="radarHeight-10" :width="18"
                :color="getColorByStatus(healthAverage)"
              >{{ healthAverage }}</v-progress-circular>
            </div>
            <div
              style="color: #b2b2b2;"
...
import HealthTrendChart from '../main/healthTrendChart.vue';
import HealthScoreTrend from '../main/healthScoreTrend.vue';
...
export default {
  ...
  computed: {
    ...
    ...mapState('statePrediction', ['deviceScore', 'healthAverage']),
    contentHeight() {
    ...
    highestPart() {
      ...
      // if (maxObj) {
      //   obj = {
      //     name: maxObj.partName,
      //     number: maxObj.partNumber,
    ...
    lowestPart() {
      ...
      if (minObj) {
        obj = {
          name: minObj.partName,
          number: minObj.partNumber,
