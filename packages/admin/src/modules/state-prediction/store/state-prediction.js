...
export default ({
  ...
  actions: {
    ...
    async getRecordList({ commit }, payload) {
      ...
      commit('setRecordList', result.rows || []);
      commit(
        'helper/setAlert',
        {
          show: true,
    ...
    // },
    // 获取数据监测列表
    async getAlgorithmModelList({ commit }, { bomId, height, width }) {
      const url = `spc/dataMonitor?bomId=${bomId}&height=${height}&width=${width}`;
      const result = await this.$spcGet(url);
      if (!result) {
        commit('setAlgorithmModelList', []);
        return;
      }
    ...
    async getAlgorithmPlcList({ commit }, bomId) {
      ...
      const { rows } = result;
      if (rows?.length) {
        commit('setAlgorithmPlcList', rows);
      } else {
        commit('setAlgorithmPlcList', []);
      }
    },
    // 获取维保任务列表
