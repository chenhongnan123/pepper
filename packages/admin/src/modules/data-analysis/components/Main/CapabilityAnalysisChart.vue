<template>
  ...
  >
    ...
    >
      ...
      >
        <highcharts
          :id="`${chartName}_processCapabilityChart`"
      ...
      >
        <!-- 处理数据模块 -->
        <v-col :cols="4">
          <p class="text-center mb-1">{{ $t('dataAnalysis.chart.processData') }}</p>
          <v-row dense no-gutters>
            <v-col :cols="6">
              <div class="chartCalData">
                <!-- 下限值 -->
                <div class="leftBottomline topline rightline">
                  <div class="smallLabelWidth ml-2">{{ $t('dataAnalysis.chart.lsl') }}</div>
                  <div class="valueWidth ml-2">{{ chartData.lsl || '*' }}</div>
                </div>
                <!-- 上限值 -->
                <div class="leftBottomline rightline">
                  <div class="smallLabelWidth ml-2">{{ $t('dataAnalysis.chart.usl') }}</div>
                  <div class="valueWidth ml-2">{{ chartData.usl || '*' }}</div>
                </div>
                <!-- 目标 -->
                <div class="leftBottomline rightline">
                  <div class="smallestLabelWidth ml-2">{{ $t('dataAnalysis.chart.target') }}</div>
                  <div class="valueWidth ml-2">{{ target || '*' }}</div>
                </div>
                <!-- 样本均值 -->
                <div class="leftBottomline rightline">
                  <div class="smallLabelWidth ml-2">
                    {{ $t('dataAnalysis.chart.sampleMean') }}
                  </div>
                ...
                <div class="leftBottomline rightline">
                  <div class="smallLabelWidth ml-2">
                    {{ $t('dataAnalysis.chart.sampleSize') }}
                  </div>
              ...
              >
                ...
                <div v-if="overallShow" class="rightBottomline topline">
                  <div class="halfLabelWidth ml-2">
                    {{ $t('dataAnalysis.chart.stDevOverall') }}
                  </div>
                ...
                >
                  <div class="halfLabelWidth ml-2">
                    {{ $t('dataAnalysis.chart.stDevWithin') }}
                  </div>
                  <div class="valueWidth ml-2">
                ...
                >
                  <div class="halfLabelWidth ml-2">
                    {{ $t('dataAnalysis.chart.stDevBetween') }}
                  </div>
                  <div class="valueWidth ml-2">
        ...
        <v-col :cols="4" v-if="overallShow">
          <p class="text-center mb-1">
            {{ $t('dataAnalysis.chart.overallCapability') }}
          </p>
          <!-- 显示：基准Z值 -->
          <v-row
            dense
          ...
          >
            <v-col :cols="6">
              <div class="chartCalData">
                ...
                >
                  <div class="largeLabelWidth ml-2">
                    {{ $t('dataAnalysis.chart.ciForZBench') }}
                  </div>
                  <div class="valueWidth ml-2">
        ...
        <v-col :cols="4" v-if="withinSubgroupShow">
          ...
          >
            <span v-if="selectedAlgorithm === 'bwxpk'">
              {{ $t('dataAnalysis.chart.betweenwithinCapability') }}
            </span>
            <span v-else>
              {{ $t('dataAnalysis.chart.withinCapability') }}
            </span>
          </p>
          <!-- 显示：基准Z值 -->
          <v-row dense v-if="z_bench" no-gutters>
            <v-col :cols="6">
              <div class="chartCalData">
                <!-- Z值 -->
                <div class="leftBottomline topline rightline">
                  <div class="smallerLabelWidth ml-2">{{ $t('dataAnalysis.chart.ZBench') }}</div>
                  <div class="valueWidth ml-2">
                ...
                <!-- Z值下限 -->
                <div class="leftBottomline rightline">
                  <div class="halfLabelWidth ml-2">{{ $t('dataAnalysis.chart.ZLSL') }}</div>
                  <div class="valueWidth ml-2">
                ...
                <!-- Z值上限 -->
                <div class="leftBottomline rightline">
                  <div class="halfLabelWidth ml-2">{{ $t('dataAnalysis.chart.ZUSL') }}</div>
                  <div class="valueWidth ml-2">
            ...
            <v-col :cols="6">
              <div class="chartCalData">
                ...
                <div class="rightBottomline topline">
                  ...
                  >{{ $t('dataAnalysis.chart.Cmk') }}</div>
                  <div
                    class="smallestLabelWidth ml-2"
                    v-else
                  >{{ $t('dataAnalysis.chart.Cpk') }}</div>
                  <div
                    v-if="selectedAlgorithm==='normxmk'"
                ...
                >
                  ...
                  >
                    {{ $t('dataAnalysis.chart.ciForCmk') }}
                  </div>
                  <div
                    v-else
                    class="halfLabelWidth ml-2"
                  >
                    {{ $t('dataAnalysis.chart.ciForCpk') }}
                  </div>
                  <div class="valueWidth ml-2">
              ...
              <div class="chartCalData">
                ...
                <div class="leftBottomline topline rightline">
                  ...
                  <div
                    v-if="selectedAlgorithm==='normxmk'"
                    class="smallestLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.Cm') }}</div>
                  <div
                    v-else
                    class="smallestLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.Cp') }}</div>
                  <div class="valueWidth ml-2">
                    {{ getFixed(chartData.z_bench_cp, 2) || '*' }}
                  </div>
                </div>
                ...
                >
                  ...
                  <div
                    v-if="selectedAlgorithm==='normxmk'"
                    class="halfLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.ciForCm') }}</div>
                  <div
                    v-else
                    class="halfLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.ciForCp') }}</div>
                  <div class="valueWidth ml-2">
                    {{ getConfidenceIntervals(chartData.LB_zbench_cp, 2) || '*' }}
                  </div>
                </div>
                ...
                <div class="leftBottomline rightline">
                  ...
                  <div
                    v-if="selectedAlgorithm==='normxmk'"
                    class="smallestLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.cml') }}</div>
                  <div
                    v-else
                    class="smallestLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.cpl') }}</div>
                  <div class="valueWidth ml-2">
                    {{ getFixed(chartData.zLSL_cpL, 2) || '*' }}
                  </div>
                </div>
                ...
                <div class="leftBottomline rightline">
                  ...
                  <div
                    v-if="selectedAlgorithm==='normxmk'"
                    class="smallestLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.cmu') }}</div>
                  <div
                    v-else
                    class="smallestLabelWidth ml-2"
                  >{{ $t('dataAnalysis.chart.cpu') }}</div>
                  <div class="valueWidth ml-2">
                    {{ getFixed(chartData.zUSL_cpU, 2) || '*' }}
                  </div>
                </div>
              ...
              <div class="chartCalData">
                ...
                >
                  ...
                  <div
                    class="halfLabelWidth ml-2"
                    v-if="selectedAlgorithm==='normxmk'"
                  >{{ $t('dataAnalysis.chart.ciForCmk') }}</div>
                  <div
                    class="halfLabelWidth ml-2"
                    v-else
                  >{{ $t('dataAnalysis.chart.ciForCpk') }}</div>
                  <div class="valueWidth ml-2">
                    <!-- 置信区间为单侧时，LB_cpk返回单个数值 -->
                    <!-- <span
                      v-if="optionsparams.options.confidenceIntervals !== 'twoSided'"
      ...
      <div
        ...
        v-if="ksigma"
      >
        {{ $t('dataAnalysis.chart.capabKsigma', { num: `${ksigma}` }) }}
      </div>
    </div>
    <v-row
      v-if="errMsgShow"
...
export default {
  ...
  computed: {
    ...
    selectedAlgorithm() {
      ...
      return this.chartData?.payload?.chart_type || '';
    },
    // 控制图实际高度
    chartHeight() {
      if (this.forceHeight) {
      ...
      handler(obj) {
        ...
        if (obj.status && obj.status === 200) {
          ...
          if (payload?.data?.length > 0) {
            [this.sourceDataPoints] = payload.data;
  ...
  mounted() {
    ...
    Highcharts.chart(`${this.chartName}_processCapabilityChart`, this.options);
  },
  methods: {
        ...
        xAxis: [
          {
            ...
            gridLineColor: this.isDark ? 'transparent' : '#ccd6eb', // y轴网格线颜色
            gridZIndex: -1,
            min: null,
            max: null,
            plotLines: [],
    ...
    getCommonChartOptions() {
      ...
      if (selectedAlgorithm === 'nnormxpk') {
        ...
        commonoptions.title.text = this.$t(
          'dataAnalysis.chart.calcBasedOn',
          { type: this.$t(`dataAnalysis.options.${dist}`) },
        );
      } else if (this.confidenceLevel) {
        // 其余算法若包含置信区间，则显示置信水平
        commonoptions.title.text = `${this.confidenceLevel}%${this.$t('dataAnalysis.options.confidenceLevel')}`;
      }
      // 高斯分布曲线的所有x轴数据
      let xAxis = [];
      ...
      if (usl || usl === 0) {
        xPlotLines.push({
          ...
          label: {
            ...
            // 与目标值一致时合并显示
            text: `${this.$t('dataAnalysis.chart.usl')}
            ${target === usl ? `,${this.$t('dataAnalysis.chart.target')}` : ''}`,
            textAlign: 'center',
            y: -5,
            rotation: 0,
          },
      ...
      if (lsl || lsl === 0) {
        xPlotLines.push({
          ...
          label: {
            ...
            // 与目标值一致时合并显示
            text: `${this.$t('dataAnalysis.chart.lsl')}
            ${target === lsl ? `,${this.$t('dataAnalysis.chart.target')}` : ''}`,
            textAlign: 'center',
            y: -5,
            rotation: 0,
          },
      ...
      if ((target || target === 0) && usl !== target && lsl !== target) {
        const targetLine = {
          ...
          label: {
            ...
            text: this.$t('dataAnalysis.chart.target'),
            textAlign: 'center',
            y: -5,
            rotation: 0,
          },
      ...
      const series = [
        {
          ...
          tooltip: {
            pointFormatter() {
              ...
              return `${that.$t('dataAnalysis.chart.value')}: ${y},${that.$t('dataAnalysis.chart.interval')}: ${xValue}<br/>`;
            },
          },
          sonification: {
            enabled: true,
      ...
      if (this.overallShow) {
        series.push({
          name: this.$t('dataAnalysis.chart.overall'),
          type: 'spline',
          data: overallPoints,
          zIndex: 1,
          dashStyle: 'solid',
      ...
      if (this.withinSubgroupShow) {
        ...
        if (selectedAlgorithm === 'bwxpk') {
          seriesName = this.$t('dataAnalysis.chart.between/within');
        } else {
          seriesName = this.$t('dataAnalysis.chart.within');
        }
        series.push({
          name: seriesName,
          type: 'spline',
...
<style scoped lang="scss">
  ...
  .chartCalData {
    ...
    text-align: left;
    .topline {
      border-top: 1px solid var(--v-info-lighten3);
    }
    .leftBottomline {
      border-left: 1px solid var(--v-info-lighten3);
