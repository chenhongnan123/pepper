<template>
  <v-container fluid>
    <portal to="app-header" v-if="pagePortalMode.workshopManagement">
        ...
        <v-card height="77" class="mb-3" elevation="0" color="bg">
          <div class="d-flex align-center px-4" style="height: 100%">
            ...
            <v-btn
              ...
              v-if="isBtnAccess('basic:workshop:remove')"
            >
            ...
            <v-btn
              ...
              v-if="isBtnAccess('basic:workshop:edit')"
            >
            ...
            <input
              ...
              v-if="isBtnAccess('basic:workshop:import')"
            />
            <v-btn
              outlined
              small
            ...
            <v-btn
              ...
              color="warning"
              @click="excelExport"
              v-if="isBtnAccess('basic:workshop:template')"
            >
              <v-icon left small>mdi-download</v-icon>
              {{ $t('workshopManagement.btn.templatedownload') }}
            </v-btn>
            <v-btn
              small
              class="text-none mr-2"
              ...
              @click="
                ...
                addDialog = true;
              "
              v-if="isBtnAccess('basic:workshop:add')"
            >
...
export default {
  ...
  data() {
    return {
      elementName: 'workshopManagement',
      workshopHeaders: [
        'no',
        // 'workshopId',
        'workshopNumber',
  ...
  computed: {
    ...
    ...mapState('workshopManagement', ['workshopList']),
  ...
  activated() {
    this.workshopManagementGrid = this.$refs.workshopManagementGrid.gridApi;
  },
  created() {
    this.getTableList();
  ...
  methods: {
    ...
    async getTableList() {
      ...
      this.options.rowData = this.workshopList;
    },
    initTableHeaders() {
      let headers = [];
      headers = this.workshopHeaders.map((item, index) => {
        const obj = {
          field: item,
          headerName: this.$t(`workshopManagement.table.${item}`),
          index,
    ...
    resetSelect() {
      if (this.workshopManagementGrid) {
        this.workshopManagementGrid.deselectAll();
      }
    },
    handleImportClick() {
    ...
    async handleDelete() {
      if (
        ...
        this.selectedTableList.forEach((item) => {
          ids += ids ? `,${item.workshopId}` : item.workshopId;
        });
        const result = await this.deleteWorkshop(ids);
        if (result) {
          this.refresh();
        }
      }
