...
export default {
  ...
  computed: {
    ...
    ...mapState('expertKnowledge', ['partList']),
    dialog: {
      get() {
  ...
  methods: {
    ...
    ...mapActions('expertKnowledge', ['addPart']),
    async handleSave() {
      if (!this.$refs.selectableBomTree.selectfloder.length) {
        this.bomErrMsg = this.$t('verification.isrequired');
        return;
      }
      console.log(this.partList, 'partList');
      console.log(this.$refs.selectableBomTree.selectfloder, 'this.$refs.selectableBomTree.selectfloder');
      const ignoreList = this.$refs.selectableBomTree.selectfloder
        .filter((item) => this.partList.some((p) => p.partId === item.partId));
      ...
      if (selectedPart.length) {
        const result = await this.addPart(selectedPart);
        if (result) {
