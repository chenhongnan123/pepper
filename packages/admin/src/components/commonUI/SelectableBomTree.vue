<template>
  <div>
    <div class="d-flex align-center" :class="isErr ? 'error--text':''">
      <span v-if="title" class="text-h6">
        {{ title }}
      </span>
      <v-checkbox
        v-model="showMaintenancePart"
        v-if="isShowMaintenancePart"
        color="primary"
        class="ml-3 mt-0"
        hide-details
        dense
        @change="handleFilter(0)"
      >
        <template #label>
          <span class="text-subtitle-2">
            {{$t('maintenance.label.showMaintenancePart')}}
          </span>
        </template>
      </v-checkbox>
      <v-spacer></v-spacer>
      <v-checkbox
        v-model="showMainPart"
        v-if="isShowMainPart"
        color="primary"
        class="mt-0"
        hide-details
        dense
        @change="handleFilter(1)"
      >
        <template #label>
          <span class="text-subtitle-2">
            {{$t('maintenance.label.showMainPart')}}
          </span>
        </template>
      </v-checkbox>
    </div>
    <div
      class="rounded-lg px-5 py-3"
      :class="isErr ? 'error-thin-border':'grey-thin-border'"
      :style="`height: ${height}px;`"
    >
      <v-text-field
        v-model="searchVal"
        append-icon="mdi-magnify"
        hide-details
        clearable
        :label="$t(`${placeholder}`)"
        @click:clear="reset"
        dense
      ></v-text-field>
      <v-treeview
        :style="`height: ${height - 60}px;overflow-y: scroll;cursor: pointer;`"
        class="my-1"
        :open-all="true"
        v-model="selectfloder"
        :active.sync="selectfloder"
        activatable
        :multiple-active="isMultiple"
        selectable
        selection-type="independent"
        selected-color="primary"
        return-object
        :search="searchVal"
        dense
        :items="bomList"
        :item-disabled="alarmDisabledKey || disabledKey"
        :item-children="itemChildren"
        :item-key="itemKey"
        :item-text="itemText"
        @update:active="handleActive"
      >
      </v-treeview>
    </div>
  </div>
</template>

<script>
export default {
  name: 'SelectableBomTree',
  data() {
    return {
      showMaintenancePart: false,
      showMainPart: false,
      searchVal: '',
      openall: true,
      selectfloder: [],
      disabledKey: '',
    };
  },
  props: {
    title: {
      type: String,
      required: false,
    },
    height: {
      type: Number,
      required: true,
    },
    bomList: {
      type: Array,
      required: true,
    },
    isMultiple: {
      type: Boolean,
    },
    isErr: {
      type: Boolean,
    },
    // 子节点
    itemChildren: {
      default: 'childNodes',
    },
    itemKey: {
      default: 'componentSequenceId',
    },
    itemText: {
      default: 'productName',
    },
    isShowMaintenancePart: {
      type: Boolean,
      default: true,
    },
    isShowMainPart: {
      type: Boolean,
      default: true,
    },
    // 表单提示词
    placeholder: {
      type: String,
      default: '搜索零部件',
    },
    // 控制 目录的disabled
    alarmDisabledKey: {
      type: String,
    },
  },
  components: {
  },
  methods: {
    handleFilter(type) {
      this.selectfloder = [];
      if (type) {
        if (this.showMaintenancePart) {
          this.showMaintenancePart = false;
        }
      } else if (this.showMainPart) {
        this.showMainPart = false;
      }
      if (!this.showMaintenancePart && !this.showMainPart) {
        this.disabledKey = '';
      } else if (this.showMaintenancePart) {
        this.disabledKey = 'isMatainDisabled';
      } else {
        this.disabledKey = 'isKeyDisabled';
      }
    },
    handleActive(item) {
      this.$emit('handleActive', item);
    },
    reset() {

    },
  },
};
</script>
