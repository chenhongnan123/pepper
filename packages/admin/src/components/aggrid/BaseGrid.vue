...
export default {
  ...
  computed: {
    ...
    ...mapState('reportsManagement', ['showChart']),
    iszh() {
    ...
    isRowSelectable() {
      if (this.options?.isRowSelectable) {
        return this.options.isRowSelectable;
      }
      return this.defaultOptions.isRowSelectable;
    },
  },
  watch: {
    rowData: {
    ...
    paginationPageSize: {
      handler(val) {
        ...
        if (this.gridApi.getModel) {
          // 不能用rowData重新计算分页，否则会出现筛选数据后更改每页行数后，分页不对的问题
          const tableRows = this.gridApi.getModel().rowsToDisplay;
          // console.log(tableRows, 'tableRows');
          this.initPagination(tableRows);
        } else {
          this.initPagination(this.rowData);
        }
        if (this.gridApi.paginationSetPageSize) {
          if (val === 0) {
  ...
  methods: {
    ...
    // 重新加载分页总数
    initPagination(rowData) {
      this.totalRecords = rowData?.length || 0;
      if (rowData?.length && this.paginationPageSize) {
        this.totalPages = Math.ceil(rowData.length / this.paginationPageSize);
      } else {
        this.totalPages = 0;
      }
      // 若当前页大于总页数，跳转到第一页
    ...
    visualizeData() {
      // 仅在报表管理页面进行表格cellRange初始化
      if (this.showChart && this.$route.name === 'reportsManagement') {
    ...
    hanldeRestoreColumn() {
      ...
      if (this.restoreColumn) {
        ...
        if (columnstate && columnstate !== 'null' && columnstate !== 'undefined') {
          this.gridColumnApi.applyColumnState({
            state: JSON.parse(columnstate),
            applyOrder: true,
          });
